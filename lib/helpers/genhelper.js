...
class GenHelper {
    generatePrivateKey() {
        let max = Buffer.from("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140", 'hex');
        let isInvalid = true;
        let privateKey;
        while (isInvalid) {
    ...
    createPrivateKeyWIF(privateKey) {
        let _t1 = Buffer.from("80" + privateKey, 'hex');
        let _t2 = sha256(_t1);
        let _t3 = sha256(Buffer.from(_t2, 'hex'));
        let checksum = _t3.substring(0, 8);
        let _t4 = _t1.toString('hex') + checksum;
        return base58.encode(Buffer.from(_t4, 'hex')).toString('hex');
    }
}
